 ...
 * @package    Backend
 */
var ClipboardMenu={initialize:function(){$$("a.cl_paste").each(function(a){a.addClass("invisible")});$$("a.clipboardmenu").each(function(b){var a=b.getAllNext("a.cl_paste");a.unshift(b);b.addEvent("contextmenu",function(c){c.preventDefault();ClipboardMenu.show(a,c)})});$(document.body).addEvent("click",function(){ClipboardMenu.hide()})},show:function(a,c){ClipboardMenu.hide();var d=new Element("div",{id:"clipboardmenu",styles:{top:(a[0].getPosition().y-6)}});var b="";a.each(function(f,e){b+='<a href="'+f.href+'" title="'+f.title+'">'+f.get("html")+" "+f.getFirst("img").alt+"</a>"});d.set("html",b);d.inject($(document.body));d.setStyle("left",a[0].getPosition().x-(d.getSize().x/2))},hide:function(){if($defined($("clipboardmenu"))){$("clipboardmenu").destroy()}}};
